	...
	public List<Message> getMessages() {
		...
		try {
			...
			while(result.moveToNext()) {
				try {
					final DateTime time = ISODateTimeFormat.dateTimeParser().parseDateTime(result.getString(result.getColumnIndex("time")));
					final String text = result.getString(result.getColumnIndex("text"));
					final Direction direction = Direction.valueOf(result.getString(result.getColumnIndex("direction")));
