        ...
        for (int i = 0; i < frameData.length; i++) {
            if (frameData[i] > 255) {
                throw new RuntimeException(
                        "Packet values must not be greater than one byte (255): " + frameData[i]);
            }
...
//		}
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug(
                    "pre-escape packet size is " + preEscapeLength + ", post-escape packet size is " + packet.length);
        }
    }
    ...
    private static int[] escapePacket(int[] packet) {
        ...
        for (int i = 1; i < packet.length; i++) {
            if (isSpecialByte(packet[i])) {
                log.debug(
                        "escapeFrameData: packet byte requires escaping byte " + ByteUtils.toBase16(
                                packet[i]));
                escapeBytes++;
            }
        }
        ...
        if (escapeBytes == 0) {
            ...
            for (int i = 1; i < packet.length; i++) {
                if (isSpecialByte(packet[i])) {
                    ...
                    log.debug("escapeFrameData: xor'd byte is 0x" + Integer.toHexString(
                            escapePacket[pos]));
                } else {
    ...
    public static boolean isSpecialByte(int b) {
        return b == SpecialByte.START_BYTE.getValue() || b == SpecialByte.ESCAPE.getValue() || b == SpecialByte.XON
                .getValue() ||
    ...
    public static int[] unEscapePacket(int[] packet) {
        ...
        if (packetEndsWithEscapeByte(packet)) {
            //packet can never end on a escape byte since the following byte is xor to unescape and will result in a array out of bounds exception
            throw new RuntimeException(
                    "Invalid packet -- packet cannot end with an escape byte " + ByteUtils.toBase16(
                            packet));
        }
