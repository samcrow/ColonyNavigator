...
public class ZNetTxRequest extends XBeeRequest {
     ...
     * @param payload
     */
    public ZNetTxRequest(int frameId, XBeeAddress64 dest64, XBeeAddress16 dest16,
                         int broadcastRadius, Option option, int[] payload) {
    ...
    public ZNetTxRequest(XBeeAddress64 dest64, int[] payload) {
        this(XBeeRequest.DEFAULT_FRAME_ID, dest64, XBeeAddress16.ZNET_BROADCAST,
                ZNetTxRequest.DEFAULT_BROADCAST_RADIUS, Option.UNICAST, payload);
    }
    ...
    protected IntArrayOutputStream getFrameDataAsIntArrayOutputStream() {
        ...
        if (this.getMaxPayloadSize() > 0 && payload.length > this.getMaxPayloadSize()) {
            throw new IllegalArgumentException(
                    "Payload exceeds user-defined maximum payload size of " + this.getMaxPayloadSize() + " bytes.  Please package into multiple packets");
        }
