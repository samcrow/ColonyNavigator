...
public class InputStreamThread implements Runnable {
                    ...
                    for (PacketListener pl : packetListenerList) {
                        try {
                            if (pl != null) {
                                ...
                                log.warn(
                                        "PacketListener is null, size is " + packetListenerList.size());
                            }
                        } catch (Throwable th) {
    ...
    private void addToResponseQueue(final XBeeResponse response) throws InterruptedException {
        ...
        while (responseQueue.size() >= conf.getMaxQueueSize()) {
            log.info(
                    "Response queue has reached the maximum size of " + conf.getMaxQueueSize() + " packets.  Trimming a packet from head of queue to make room");
            responseQueue.poll();
        }
...
//								log.debug("Received packet: int[] packet = {" + ByteUtils.toBase16(response.getRawPacketBytes(), ", ") + "};");	
                            ...
                            log.warn("expected start byte but got this " + ByteUtils.toBase16(
                                    val) + ", discarding");
                        }
                    } else {
