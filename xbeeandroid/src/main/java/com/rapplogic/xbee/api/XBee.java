...
public class XBee implements IXBee {
    ...
    private void doStartupChecks() throws XBeeException {
        ...
        try {
            ...
            if (ap.getValue()[0] != 2) {
                log.warn(
                        "XBee radio is in API mode without escape characters (AP=1).  The radio must be configured in API mode with escape bytes (AP=2) for use with this library.");
                ...
                if (ap.isOk()) {
                    log.info(
                            "Successfully set AP mode to 2.  This setting will not persist a power cycle without the WR (write) command");
                } else {
            ...
            throw new XBeeException(
                    "AT command timed-out while attempt to set/read in API mode.  Check that the XBee radio is in API mode (AP=2); it will not function propertly in AP=1");
        }
    }
     ...
     */
    public void open(FT_Device device) throws XBeeException {
        try {
            if (this.isConnected()) {
                throw new IllegalStateException(
                        "Cannot open new connection -- existing connection is still open.  Please close first");
            }

            this.type = null;

            SerialPortConnection serial = new SerialPortConnection();
            serial.openSerialPort(device);
    ...
    public void sendRequest(XBeeRequest request) throws IOException {
        if (this.type != null) {
            ...
            if (type == RadioType.SERIES1 && request.getClass()
                    .getPackage()
                    .getName()
                    .indexOf("api.zigbee") > -1) {
                throw new IllegalArgumentException(
                        "You are connected to a Series 1 radio but attempting to send Series 2 requests");
            } else if (type == RadioType.SERIES2 && request.getClass()
                    .getPackage()
                    .getName()
                    .indexOf("api.wpan") > -1) {
                throw new IllegalArgumentException(
                        "You are connected to a Series 2 radio but attempting to send Series 1 requests");
            }
        }
     ...
     */
    public XBeeResponse sendSynchronous(final XBeeRequest xbeeRequest, int timeout)
            throws XBeeTimeoutException, XBeeException {
        if (xbeeRequest.getFrameId() == XBeeRequest.NO_RESPONSE_FRAME_ID) {
            throw new XBeeException(
                    "Frame Id cannot be 0 for a synchronous call -- it will always timeout as there is no response!");
        }
        ...
        try {
            ...
            pl = new PacketListener() {
                ...
                public void processResponse(XBeeResponse response) {
                    if (response instanceof XBeeFrameIdResponse && ((XBeeFrameIdResponse) response).getFrameId() == xbeeRequest
                            .getFrameId()) {
     ...
     */
    public XBeeResponse sendSynchronous(final XBeeRequest request)
            throws XBeeTimeoutException, XBeeException {
...
//	}
    ...
    private XBeeResponse getResponseTimeout(Integer timeout)
            throws XBeeException, XBeeTimeoutException {
     ...
     * @throws XBeeException
     */
    public List<? extends XBeeResponse> collectResponses(int wait, CollectTerminator terminator)
            throws XBeeException {
    ...
    public boolean isConnected() {
        try {
            if (parser.getXBeeConnection().getInputStream() != null && parser.getXBeeConnection()
