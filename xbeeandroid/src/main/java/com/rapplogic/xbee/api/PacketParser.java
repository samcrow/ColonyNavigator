...
public class PacketParser implements IIntInputStream, IPacketParser {
    ...
    static {
        ...
        handlerMap.put(ApiId.ZNET_IO_NODE_IDENTIFIER_RESPONSE.getValue(),
                ZNetNodeIdentificationResponse.class);
    ...
    static void registerResponseHandler(int apiId, Class<? extends XBeeResponse> clazz) {
        if (handlerMap.get(apiId) == null) {
            log.info(
                    "Registering response handler " + clazz.getCanonicalName() + " for apiId: " + apiId);
        } else {
            log.warn("Overriding existing implementation: " + handlerMap.get(apiId)
                    .getCanonicalName() + ", with " + clazz.getCanonicalName() + " for apiId: " + apiId);
        }
    ...
    static void unRegisterResponseHandler(int apiId) {
        if (handlerMap.get(apiId) != null) {
            log.info("Unregistering response handler " + handlerMap.get(apiId)
                    .getCanonicalName() + " for apiId: " + apiId);
        ...
        try {
            ...
            for (Integer handlerApiId : handlerMap.keySet()) {
                if (intApiId == handlerApiId) {
                    log.debug("Found response handler for apiId [" + ByteUtils.toBase16(
                            intApiId) + "]: " + handlerMap.get(handlerApiId).getCanonicalName());
            ...
            if (response == null) {
                log.info("Did not find a response handler for ApiId [" + ByteUtils.toBase16(
                        intApiId) + "].  Returning GenericResponse");
            ...
            if (!this.isDone()) {
                throw new XBeeParseException(
                        "There are remaining bytes according to stated packet length but we have read all the bytes we thought were required for this packet (if that makes sense)");
            }
            ...
            log.error(
                    "Failed due to exception.  Returning ErrorResponse.  bytes read: " + ByteUtils.toBase16(
                            rawBytes.getIntArray()), e);
            exception = e;
    ...
    public int read() throws IOException {
        ...
        if (XBeePacket.isSpecialByte(b)) {
            ...
            if (b == XBeePacket.SpecialByte.ESCAPE.getValue()) {
                ...
                log.warn("Found unescaped special byte " + ByteUtils.formatByte(
                        b) + " at position " + bytesRead);
            }
        }
        ...
        if (bytesRead > 2) {
            ...
            log.debug("Read byte " + ByteUtils.formatByte(
                    b) + " at position " + bytesRead + ", packet length is " + this.length.get16BitValue() + ", #escapeBytes is " + escapeBytes + ", remaining bytes is " + this
                    .getRemainingBytes());
            ...
            if (this.getFrameDataBytesRead() >= (length.get16BitValue() + 1)) {
                ...
                if (!checksum.verify()) {
                    throw new XBeeParseException(
                            "Checksum is incorrect.  Expected 0xff, but got " + checksum.getChecksum());
                }
            }
        }
