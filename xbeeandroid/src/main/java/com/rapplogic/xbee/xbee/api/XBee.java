...
import com.rapplogic.xbee.xbee.api.HardwareVersion.RadioType;
...
public class XBee implements IXBee {
	...
	private void doStartupChecks() throws XBeeException {
		...
		try {				
			...
			throw new XBeeException("AT command timed-out while attempt to set/read in API mode.  Check that the XBee radio is in API mode (AP=2); it will not function properly in AP=1");
		}		
	}
	 ...
	 */
	public void open(XBeeConnection connection) throws XBeeException {
		try {
			if (this.isConnected()) {
