...
public class Download {
     ...
     *
     * @param remoteAddress The address to download the file from
     */
     ...
     *
     * @param remoteAddress    The address to download the file from
     * @param downloadLocation The local location to save the file to
     */
     ...
     * @throws IllegalStateException If the download is not finished
     */
        ...
        switch (state.statusCode) {
            ...
            default:
                throw new IllegalStateException(
                        "DownloadManager provided an invalid status code value: " + state.statusCode);
        }
    }
    ...
    private void refreshState() {
        Cursor cursor = downloadManager.query(
                new DownloadManager.Query().setFilterById(state.downloadId));
        if (cursor.getCount() > 0) {
            // Move to row 0
            cursor.moveToNext();

            // Get progress
            final int newBytesDownloaded = cursor.getInt(
                    cursor.getColumnIndex(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));
            final int newTotalBytes = cursor.getInt(
            ...
            final int newStatusCode = cursor.getInt(
